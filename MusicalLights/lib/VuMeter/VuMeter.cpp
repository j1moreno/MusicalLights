...
class VuMeter {
    ...
    public:
        VuMeter(int numberOfLevels, int startingPin, int zero_value);
...
VuMeter::VuMeter(int numberOfLevels, int startingPin, int zero_value = ZERO_VALUE) {
...
int VuMeter::GetNumberOfLevels() {
    return number_of_levels_;
}

void VuMeter::CycleLevels(int delay_between_levels) {
    for (int i = 0; i < number_of_levels_; i++) {
...
bool VuMeter::ExceedsZeroCount(int audio_level)
{
...
void VuMeter::RedefineThresholds(int audio_level)
{
...
void VuMeter::LedReact(int audio_level){
...
void VuMeter::DisplayAudioLevel(int audio_level) {
    Serial.print(audio_level);
    Serial.print("\t");
...
void VuMeter::DisplayAudioLevelBasic(int audio_level) {
    if (audio_level == 0) {
        for (int i = 0; i < number_of_levels_; i++) {
        digitalWrite(led_pins_[i], LOW);
        }
